...
def at_fix_now(reader, dry, lookback=4.0):
    """True if the unit's recent output shows a baseline fix (not no-fix)."""
    if dry:
        return True
    ...
    try:
        if not args.dry_run:
            ...
            if not ensure_baseline_fix(gps, reader, args.dry_run, MB):
                print("!! ABORT: unit never reached a baseline fix. Check serial ports, GPS\n"
                      "   injection wiring, and that --max-baseline-s exceeds the unit's\n"
                      "   reacquisition time. No trials run.", flush=True)
                return

...
def _utc_fields():
    """Live UTC hhmmss.ss and ddmmyy for RMC/GGA. Some units (DY) reject a stale or
    non-advancing timestamp and will not go to a fix; using current UTC fixes that."""
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc)
    return now.strftime("%H%M%S.00"), now.strftime("%d%m%y")


def gps_batch(lat, lon, sog=12.0, cog=90.0):
    """Valid GGA/RMC/VTG batch at a position (the proven-accepted set), with a LIVE
    UTC timestamp/date so strict units accept the fix."""
    ...
    tm, dt = _utc_fields()
    out = b""
    out += sentence(f"GPGGA,{tm},{la},{lah},{lo},{loh},1,08,1.0,10.0,M,,M,,".encode())
...
# spoof-position RMC body (base for malformation). Built fresh per call with a LIVE
# timestamp so the ONLY anomaly is the malformation under test, not a stale time that
# a strict unit (DY) would reject for the wrong reason.
def spoof_rmc_body():
    tm, dt = _utc_fields()
    return f"GPRMC,{tm},A,4330.0000,N,07130.0000,W,0.0,90.0,{dt},,,A".encode()

# field name -> comma-split index (same layout as spoof_rmc_body)
...
def arm1_cases():
    ...
    cases["rsv_dollar_in_data"] = sentence(
        f"GPRMC,{tm[:4]}${tm[4:]},A,4330.0000,N,07130.0000,W,0,0,{dt},A,,A".encode())
    # bare-LF terminator (accepted in an earlier run)
    cases["struct_only_lf"] = b"$" + body + b"*" + cksum(body) + b"\n"
    # two start delimiters / doubled end - structural ambiguity
    cases["struct_multi_end"] = b"$" + body + b"*" + cksum(body) + b"\r\n\r\n"
    # checksum computed without the commas (plausible-but-wrong)
    nocommas = body.replace(b",", b"")
    cases["spec_cks_excludes_commas"] = b"$" + body + b"*" + cksum(nocommas) + b"\r\n"
    # ---- negative controls: should ALWAYS be rejected ----
    cases["NEG_no_start_delim"] = body + b"*" + cksum(body) + b"\r\n"
    cases["NEG_cks_wrong_value"] = b"$" + body + b"*00\r\n"
...
def main():
    ...
    if not args.dry_run and args.gps == args.ais:
        print(f"!! ABORT: --gps and --ais are the SAME port ({args.ais}). They must be two\n"
              f"   different serial adapters (GPS input vs AIS output).", flush=True)
        reader.stop(); return
    gps = None
    if not (args.dry_run or not HAVE_SERIAL):
        try:
            gps = serial.Serial(args.gps, GPS_BAUD, timeout=1)
        except Exception as e:
            print(f"!! ABORT: could not open GPS port {args.gps}: {e}", flush=True)
            reader.stop(); return
        # confirm opening/using the GPS port did not knock the AIS reader offline
        before = reader.total_lines
        for _ in range(5):
            gps_writer(gps, args.dry_run, gps_batch(BASE_LAT, BASE_LON)); time.sleep(1.0)
        if reader.total_lines == before:
            print(f"!! ABORT: AIS reader went silent the moment the GPS port was used\n"
                  f"   (was {before} lines, still {before}). Opening/writing {args.gps} is\n"
                  f"   disrupting {args.ais} - likely both adapters share a USB hub that browns\n"
                  f"   out, or the ttyUSB numbers swapped on replug. Use stable\n"
                  f"   /dev/serial/by-id/ paths for --gps and --ais, or separate USB hubs.",
                  flush=True)
            reader.stop(); gps.close(); return
        print(f"  GPS port open; AIS still live ({reader.total_lines} lines). Proceeding.",
              flush=True)

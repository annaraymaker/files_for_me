...
def build_dtm():
    """WGS84 datum sentence, no offsets. Built raw (pynmea2 has no clean DTM ctor)."""
    body = "GPDTM,W84,,0.0,N,0.0,E,0.0,W84"
...
def build_sentences(lat, lon, alt, speed_kts, course_deg):
    ...
    # ZDA: time, day, month, year, local-zone-hours, local-zone-minutes
    zda = pynmea2.ZDA('GP','ZDA',(t, now.strftime("%d"), now.strftime("%m"),
          now.strftime("%Y"), '00','00'))
    return (gga, rmc, vtg, zda)
...
def main():
    ...
    p.add_argument("--no-dtm", action="store_true", help="do not send the DTM datum sentence")
    ...
    print(f"[{args.mode}] -> {args.port} @ {args.baud}, rate {args.rate}s, "
          f"DTM={'off' if args.no_dtm else 'on'}. Ctrl-C to stop.")
        ...
        while True:
            for lat, lon, alt, spd, crs in args.gen(args):
                ...
                for sent in build_sentences(lat, lon, alt, spd, crs):
                    ser.write((str(sent) + '\r\n').encode())
                if not args.quiet:
                    print(f"  {lat:.6f},{lon:.6f}  spd={spd:.1f}kt  brg={crs:.1f}deg")

...
def main():
    ...
    ap.add_argument("--dd-baseline-s", type=float, default=8.0)
    ap.add_argument("--dd-observe-s", type=float, default=6.0)
    ap.add_argument("--dd-recovery-s", type=float, default=8.0)
    ...
    try:
        ...
        if not args.skip_deepdive:
            cmd = [py, "-u", "run_repeatability.py", "--gps", args.gps, "--ais", args.ais,
                   ...
                   "--baseline-s", str(args.dd_baseline_s),
                   "--observe-s", str(args.dd_observe_s),
                   "--recovery-s", str(args.dd_recovery_s)]
            if args.dry_run:
                cmd.append("--dry-run")
